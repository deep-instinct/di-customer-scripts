# Deep Instinct v4 REST API Wrapper for Python
# Original Version Created by: Patrick Van Zandt
# Maintained and updated by Deep Instinct Professional Services
#
# Compatibility:
# -Deep Instinct D-Appliance versions 4.0.x, 3.x
# -Written and tested using a Python 3.8.8 instance installed by Anaconda
#
# Suggested Usage:
# 1. Save this file as deepinstinct4.py in the same directory as your code
# 2. Include "import deepinstinct4 as di" at the top of your code
...
#Returns list of RPM Agent Versions
def get_rpm_agent_version():
    #get data
    headers = {'accept': 'application/json', 'Authorization': key}
    request_url = f'https://{fqdn}/api/v1/deployment/armed-rpms/agent-versions'
    response = requests.get(request_url, headers=headers)
...
#Creates an armed rpm package
def create_armed_rpm(tenant_id, agent_version_id, no_initial_full_scan, agent_disabled_after_installation, tag, abort_if_no_server_connection, randomize_initial_full_scan, extraction_path):
    #get data
    headers = {'accept': 'application/json', 'Authorization': key}
    request_url = f'https://{fqdn}/api/v1/deployment/armed-rpms/'
    payload = {'tenant_id': tenant_id, 'agent_version_id': agent_version_id, 'extraction_path': extraction_path, 'tag': tag, 'randomize_initial_full_scan': randomize_initial_full_scan, 'no_initial_full_scan': no_initial_full_scan, 'agent_disabled_after_installation': agent_disabled_after_installation, 'abort_if_no_server_connection': abort_if_no_server_connection}
    ...
    if response.status_code == 200:
        return response.json()
        #print("Armed rpm id: ", data['armed_rpm_id'])
    elif response.status_code == 400:
        print('Bad Request')
    else:
        return []

#Gets status of armed rpm by ID
def get_armed_rpm_status(armed_rpm_id):
    headers = {'accept': 'application/json', 'Authorization': key}
    request_url = f'https://{fqdn}/api/v1/deployment/armed-rpms/{armed_rpm_id}'
    payload = {'armed_rpm_id': armed_rpm_id}
...
#Downloads armed rpm file by ID
def download_armed_rpm(armed_rpm_id):
    headers = {'accept': 'application/json', 'Authorization': key}
    request_url = f'https://{fqdn}/api/v1/deployment/armed-rpms/{armed_rpm_id}/download'
    payload = {'armed_rpm_id': armed_rpm_id}
...
# Returns a list of all visible Devices (bulk get)
...
# Returns devices one at a time
def get_devices_by_one():

    headers = {'accept': 'application/json', 'Authorization': key}
    last_id = int(input('Enter highest Device ID: '))
    error_count = 0
    collected_devices = []

    for device_id in range(1, last_id):
        ...
        if response.status_code == 200:
            ...
            if not quiet_mode:
                print('Returned record for device', (device_id))
            #for device in devices:
            if devices['license_status'] == 'ACTIVATED':
                collected_devices.append(devices)
...
# Create Groups
def create_group(name, policy_id, msp_id):
    # Calculate headers, URL, and payload
    headers = {'Content-Type': 'application/json', 'Authorization': key}
    request_url = f'https://{fqdn}/api/v1/groups/'
    payload = {'name': name, 'policy_id': policy_id, 'msp_id': msp_id}
    ...
    if response.status_code == 200:
        ...
        print('ERROR: Policy ID is not found')
        return False
    elif response.status_code == 409:
        print('ERROR: Group name already exists')
        return False
    elif response.status_code == 422:
        print('ERROR: The length must be between 3 and 35 characters')
    else:
...
def put_windows_policy_data(automatic_upgrade, automatic_brain_upgrade,uninstall_password_hash, disable_password_hash, enable_schedule_scan,scheduled_scan_mode, scheduled_scan_day_of_week, scheduled_scan_day_of_month, scheduled_scan_week, scheduled_scan_time, hide_d_client_ui, prevention_level, detection_level, scan_network_drives, powershell_script_action, html_applications_action, activescript_action, prevent_all_activescript_usage, office_macro_script_action, protection_level_pua, action_on_prevented_files, malicious_js_command_execution, ransomware_behavior, in_memory_protection, remote_code_injection, known_payload_execution, arbitrary_shellcode_execution, process_tampering, reflective_dll_loading, amsi_bypass, direct_system_calls, credentials_dump, deep_memory_scanning, dual_use, gradual_deployment, suspicious_activity_detection, suspicious_powershell_command_execution, malicious_powershell_command_execution, suspicious_script_execution, embedded_dde_object_in_office_document, integrate_agent_with_windows_security_center, network_isolated_permitted_connections, enable_dcloud_services, model):
    headers = {'accept': 'application/json', 'Authorization': key}
    request_url = f'https://{fqdn}/api/v1/policies/{policy_id}/data'
    data = {'automatic_upgrade': automatic_upgrade,'automatic_brain_upgrade': automatic_brain_upgrade, 'uninstall_password_hash': uninstall_password_hash, 'disable_password_hash': disable_password_hash, 'enable_scheduled_scan': enable_schedule_scan, 'scheduled_scan_mode': scheduled_scan_mode, 'scheduled_scan_day_of_week': scheduled_scan_of_week, 'scheduled_scan_day_of_month': scheduled_scan_day_of_month, 'scheduled_scan_week': scheduled_scan_week, 'scheduled_scan_time': scheduled_scan_time, 'hide_d_client_ui': hide_d_client_ui, 'prevention_level': prevention_level, 'detection_level': detection_level, 'scan_network_drives': scan_network_drives, 'powershell_script_action': powershell_script_action, 'html_applications_action': html_applications_action, 'activescript_action': activescript_action, 'prevent_all_activescript_usage': prevent_all_activescript_usage, 'office_macro_script_action': office_macro_script_action, 'protection_level_pua': protection_level_pua, 'action_on_prevented_files': action_on_prevented_files, 'malicious_js_command_execution': malicious_js_command_execution, 'ransomware_behavior': ransomware_behavior, 'in_memory_protection': in_memory_protection, 'remote_code_injection': remote_code_injection, 'known_payload_execution': known_payload_execution, 'arbitrary_shellcode_execution': arbitrary_shellcode_execution, 'process_tampering': process_tampering, 'reflective_dll_loading': reflective_dll_loading, 'reflective_dotnet_injection': reflective_dll_loading, 'amsi_bypass':amsi_bypass, 'direct_system_calls': direct_system_calls, 'credentials_dump': credentials_dump, 'deep_memory_scanning': deep_memory_scanning, 'dual_use': dual_use, 'gradual_deployment': gradual_deployment, 'suspicious_activity_detection': suspicious_activity_detection, 'suspicious_powershell_command_execution': suspicious_powershell_command_execution, 'malicious_powershell_command_execution': malicious_powershell_command_execution, 'suspicious_script_execution': suspicious_script_execution, 'embedded_dde_object_in_office_document': embedded_dde_object_in_office_document, 'integrate_agent_with_windows_security_center': integrate_agent_with_windows_security_center, 'network_isolated_permitted_connections': network_isolated_permitted_connections, "enable_dcloud_services": enable_dcloud_services, 'model': model} 
...
def create_api_key_msp(name, permission, msp_id):
    # Calculate headers, URL, and payload
    headers = {'Content-Type': 'application/json', 'Authorization': key}
    request_url = f'https://{fqdn}/api/v1/api-connectors/msp-connector'
    payload = {'name': name, 'permission': permission, "msp_id": msp_id}
    ...
    if response.status_code == 200:
        ...
        print('ERROR: MSP not found')
        return False
    elif response.status_code == 409:
        print('ERROR: Connector key already exists')
    else:
        return 'ERROR: Unexpected return code '+ str(response.status_code)
...
# NOTE 1: Note that even when using exact hostnames, there
#         may be multiple matches found, and in that case all matching regex_hostname_search
#         will have their tag updated.
# NOTE 2: If group rules using TAGS is in place, updating device tags will cause
#         related groups to be updated immediately.
def update_tag(search_list=False, new_tag=''):
    # Get device IDs
    device_ids = get_device_ids(search_list=search_list, regex_hostname_search=regex_hostname_search, cidr_search=cidr_search)
    # Lookup to get Device Group ID
    response = requests.post(request_url, headers=headers, json=payload)
    for device in device_ids:
        request_url = f'https://{fqdn}/api/v1/devices/{device_id}/tag'
        headers = {'accept': 'application/json', 'Authorization': key}
        payload = {'comment': new_tag}
...
def change_dict_key(dictionary, old_key, new_key, default_value=None):
    dictionary[new_key] = d.pop(old_key, default_value)
